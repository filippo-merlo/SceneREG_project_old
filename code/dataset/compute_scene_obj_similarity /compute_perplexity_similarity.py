...
tokenizer = AutoTokenizer.from_pretrained(model_id, cache_dir=CACHE_DIR, download = True, token=ACCESS_TOKEN)
model = AutoModelForCausalLM.from_pretrained(model_id, torch_dtype=dtype, device_map=device, cache_dir=CACHE_DIR, token=ACCESS_TOKEN, download = True)
...
for scene_name in scenes_categories[:2]:
    prompt = f"In a {scene_name} there is a"
    results = generate_ranking(prompt, candidates[:10], model=model, tokenizer=tokenizer)
